...
import type { CanvasItem, EmbedItem, MediaItem, StackRecord } from '../types/canvas'
import { expandStackSelection, hitStackGroupAt, placeItemsTight, screenToWorld, stackCollapsedSnapBounds, stackGroupBounds } from '../utils/layout'
import { collapsedStackFanCards, collapsedStackFolderBounds, collectItemsInStackTree, containerOf, itemsInContainer } from '../utils/stacks'
...
import { applyModalTransform, beginModalTransform, type ModalTransformSession } from '../utils/modalTransform'
...
export function useInfiniteCanvasController() {
  ...
  /**
   * Freeze sibling-stack geometry for the lifetime of one navigation.
   *
   * Exit updates `items` and `stacks` every RAF while gathering the stack being
   * left. Sibling stacks cannot change while navigation is locked, so rebuilding
   * every recursive fan on those updates only creates fresh React props and
   * needless media/compositor work. The false/null animation state between two
   * navigations invalidates this memo, including when the same stack is revisited.
   */
  const parentPeerStackSnapshots = useMemo(() => {
    ...
    // Deliberately snapshot at animation start. `items` / `stacks` change on
    // every exit frame, while sibling-stack geometry is navigation-locked.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    isEnterAnim,
    isExitAnim,
  ...
  /**
   * Parent peer stacks follow the same ownership rule as ordinary peer items:
   * ghost while still inside the navigation stack, real parent layer after
   * handoff. Keeping the ghost alive across handoff changes its coordinate space
   * and causes a second fan/compositor pass that reads as a slow flash.
   */
